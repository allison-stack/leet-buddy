...
describe('TimerManager snapshot elapsedMs', () => {
  ...
  it('excludes paused time from elapsed', () => {
    ...
    tm.pause(1, 10000);
    tm.resume(1, 20000); // 10s paused
    const snap = tm.snapshot(1, 50000); // 50s total, 10s paused → 40s elapsed
  ...
  it('freezes elapsed when markSolved is called', () => {
    ...
    const snap = tm.snapshot(1, 90000); // time advances past solve, elapsed stays frozen
