...
import { HINT_CACHE_MAX_ENTRIES, INTERVIEW_HOURLY_CAP } from '@/shared/constants';
...
export async function getState(): Promise<WorkerState> {
  ...
  const interviewLimiterJson = await getLocal<number[]>('interview_rate_limit');
  ...
  cached = {
    ...
    interviewLimiter: new RateLimiter(INTERVIEW_HOURLY_CAP, 60 * 60 * 1000, interviewLimiterJson ?? []),
  };
  return cached;
}
...
export async function persistInterviewLimiter(state: WorkerState): Promise<void> {
  await setLocal('interview_rate_limit', state.interviewLimiter.toJSON());
}
